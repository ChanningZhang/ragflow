    ...
    def _parse_single_image(
        ...
        if prompt_mode in ['prompt_layout_all_en', 'prompt_layout_only_en', 'prompt_grounding_ocr']:
            ...
            if filtered and prompt_mode != 'prompt_layout_only_en':  # model output json failed, use filtered process
                ...
                origin_image.save(image_buffer, format='JPEG')
                image_data = image_buffer.getvalue()
                
                result.update({
                    'layout_info_data': response,  # 直接使用内存中的响应数据
                    'layout_image_data': image_data,  # 内存中的图片数据
                })

                result.update({
                    'md_content_data': cells  # 直接使用内存中的 Markdown 数据
                })
                result.update({
                    'filtered': True
                })
                ...
                import io
                image_buffer = io.BytesIO()
                image_with_layout.save(image_buffer, format='JPEG')
                image_data = image_buffer.getvalue()
                
                result.update({
                    'layout_info_data': cells,  # 直接使用内存中的 JSON 数据
                    'layout_image_data': image_data,  # 内存中的图片数据
                })
                if prompt_mode != "prompt_layout_only_en":  # no text md when detection only
                    ...
                    result.update({
                        'md_content_data': md_content,  # 直接使用内存中的 Markdown 数据
                        'md_content_nohf_data': md_content_no_hf,  # 直接使用内存中的无页眉页脚 Markdown 数据
                    })
        else:
            # 将图片转换为内存中的字节数据
            import io
            ...
            result.update({
                'layout_image_data': image_data,  # 内存中的图片数据
                'md_content_data': response,  # 直接使用内存中的 Markdown 数据
            })
