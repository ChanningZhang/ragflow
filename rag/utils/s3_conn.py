...
class RAGFlowS3:
    ...
    def copy_object(self, source_bucket, source_object, dest_bucket, dest_object):
        """
        Copy an object from source bucket to destination bucket within S3.
        ...
        for _ in range(3):
            try:
                ...
                if self.prefix_path:
                    actual_source_object = f"{self.prefix_path}/{source_bucket}/{source_object}"
                    actual_dest_object = f"{self.prefix_path}/{dest_bucket}/{dest_object}"
                else:
                ...
                # Use S3 copy_object API for server-side copy
                copy_source = {
